...
| Code | Explanation |
| :--- | :--- |
| ``` SEE BELOW``` | Uses `CREATE_PLAN` to define the new resource plan named `WORKLOAD_PLAN`. |

```sql
BEGIN
  DBMS_RESOURCE_MANAGER.CREATE_PLAN(
    plan    => 'WORKLOAD_PLAN',
    comment => 'Resource plan for workload management'
  );
END;
/
```
...
| Code | Explanation |
| :--- | :--- |
| ``` See below``` | Uses `CREATE_CONSUMER_GROUP` to define a new group. |

```sql
BEGIN
  DBMS_RESOURCE_MANAGER.CREATE_CONSUMER_GROUP(
    consumer_group => 'HIGH_PRIORITY_GROUP',
    comment        => 'Group for critical OLTP tasks'
  );
END;
/
```
...
| Code | Explanation |
| :--- | :--- |
| ``` See Below``` | Uses `CREATE_PLAN_DIRECTIVE` to set rules. `cpu_p1` sets the percentage share of CPU at priority level 1 (P1). `active_sess_pool_p1` limits the number of concurrent active sessions for the group. **Note**: The sum of all `cpu_p1` values must equal 100. |


```sql
-- Directive for the High Priority Group
BEGIN
  DBMS_RESOURCE_MANAGER.CREATE_PLAN_DIRECTIVE(
    plan                => 'WORKLOAD_PLAN',
    group_or_subplan    => 'HIGH_PRIORITY_GROUP',
    comment             => 'Directive for high-priority group',
    cpu_p1              => 75,
...
-- Directive for Other Sessions
BEGIN
  DBMS_RESOURCE_MANAGER.CREATE_PLAN_DIRECTIVE(
    plan             => 'WORKLOAD_PLAN',
    group_or_subplan => 'OTHER_GROUPS',
    comment          => 'Default low priority tasks',
    cpu_p1           => 25
  );
END;
/
...
| Code | Explanation |
| :--- | :--- |
| ``` SEE Below``` | The `ALTER SYSTEM` command sets the resource manager plan for the database instance. Using the package provides a programmatic way to activate the plan. |


```sql
-- Using ALTER SYSTEM command
ALTER SYSTEM SET RESOURCE_MANAGER_PLAN = 'WORKLOAD_PLAN';

-- Using the package
BEGIN
  DBMS_RESOURCE_MANAGER.SWITCH_PLAN('WORKLOAD_PLAN');
END;
/
```
